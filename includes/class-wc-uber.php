...
class WC_Uber_Shipping_Method extends WC_Shipping_Method {
    ...
    /**
     * Format address as a single string for Uber API
     */
    private function format_address_string($address_data) {
        $parts = [];
        if (!empty($address_data['address'])) $parts[] = $address_data['address'];
        if (!empty($address_data['address_2'])) $parts[] = $address_data['address_2'];
        if (!empty($address_data['city'])) $parts[] = $address_data['city'];
        if (!empty($address_data['state'])) $parts[] = $address_data['state'];
        if (!empty($address_data['postcode'])) $parts[] = $address_data['postcode'];
        if (!empty($address_data['country'])) $parts[] = $address_data['country'];
    ...
    public function calculate_shipping($package = []): void {
        ...
        // Create address strings for Uber API
        $dropoff_address = $this->format_address_string($package['destination']);
        $pickup_address = $this->format_address_string([
